...
public class OutboxService {
    ...
    public void recordPaymentEvent(Payment payment, OutboxEventType eventType) {
        PaymentEventPayload payload = PaymentEventPayload.from(payment);

        try {
            String payloadJson = objectMapper.writeValueAsString(payload);

            OutboxEvent event = OutboxEvent.pendingPayment(payment.getId(), eventType, payloadJson);

            outboxEventRepository.save(event);
        } catch (JacksonException exception) {
            throw new IllegalStateException("Cannot serialize payment outbox event", exception);
        }
    }
