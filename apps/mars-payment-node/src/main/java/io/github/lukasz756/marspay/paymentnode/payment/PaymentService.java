...
public class PaymentService {
    ...
    PaymentService(PaymentRepository paymentRepository, BalanceAccountRepository balanceAccountRepository,
                   PaymentOperationRepository paymentOperationRepository, LedgerService ledgerService,
    ...
    @Transactional
    public Payment createPayment(UUID sourceBalanceAccountId, UUID targetBalanceAccountId, long amountMinor,
                                 String reference) {
        BalanceAccount sourceBalanceAccount =
                balanceAccountRepository.findById(sourceBalanceAccountId)
                        .orElseThrow(() -> new BalanceAccountNotFoundException(sourceBalanceAccountId));
        BalanceAccount targetBalanceAccount =
                balanceAccountRepository.findById(targetBalanceAccountId)
                        .orElseThrow(() -> new BalanceAccountNotFoundException(targetBalanceAccountId));
        ...
        if (!Objects.equals(sourceCurrency, targetCurrency)) {
            throw new PaymentCurrencyMismatchException(sourceBalanceAccountId, sourceCurrency, targetBalanceAccountId
                    , targetCurrency);
        }

        Payment payment = Payment.create(sourceBalanceAccountId, targetBalanceAccountId, amountMinor, sourceCurrency,
                                         reference);

        if (paymentRepository.existsBySourceBalanceAccountIdAndReference(sourceBalanceAccountId,
                                                                         payment.getReference())) {
            throw new PaymentAlreadyExistsException(sourceBalanceAccountId, payment.getReference());
        }
        Payment savedPayment = paymentRepository.saveAndFlush(payment);

        recordOperation(savedPayment, PaymentOperationType.CREATE);

        outboxService.recordPaymentEvent(savedPayment, OutboxEventType.PAYMENT_CREATED);
    ...
    public Payment requestAuthorization(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.AUTHORIZATION_REQUESTED);

        ledgerService.recordPaymentMovement(payment.getId(), LedgerTransactionType.PAYMENT_AUTHORIZE,
                                            payment.getCurrency(), payment.getReference(), sourceAccount.getId(),
                                            LedgerBalanceBucket.AVAILABLE,
                                            sourceAccount.getId(), LedgerBalanceBucket.RESERVED,
                                            payment.getAmountMinor());

        outboxService.recordPaymentEvent(payment, OutboxEventType.PAYMENT_AUTHORIZATION_REQUESTED);
    ...
    public Payment confirmAuthorization(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.AUTHORIZATION_CONFIRMED);
    ...
    public Payment declineAuthorization(UUID paymentId) {
        ...
        ledgerService.recordPaymentMovement(payment.getId(), LedgerTransactionType.PAYMENT_AUTHORIZATION_DECLINED,
                                            ...
                                            sourceAccount.getId(), LedgerBalanceBucket.AVAILABLE,
                                            payment.getAmountMinor());
    ...
    public Payment requestCapture(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.CAPTURE_REQUESTED);

        outboxService.recordPaymentEvent(payment, OutboxEventType.PAYMENT_CAPTURE_REQUESTED);
    ...
    public Payment confirmCapture(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.CAPTURE_CONFIRMED);

        ledgerService.recordPaymentMovement(payment.getId(), LedgerTransactionType.PAYMENT_CAPTURE,
                                            payment.getCurrency(), payment.getReference(), sourceAccount.getId(),
                                            LedgerBalanceBucket.RESERVED,
                                            targetAccount.getId(), LedgerBalanceBucket.AVAILABLE,
                                            payment.getAmountMinor());
    ...
    public Payment requestCancel(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.CANCEL_REQUESTED);

        outboxService.recordPaymentEvent(payment, OutboxEventType.PAYMENT_CANCEL_REQUESTED);
    ...
    public Payment confirmCancel(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.CANCEL_CONFIRMED);

        ledgerService.recordPaymentMovement(payment.getId(), LedgerTransactionType.PAYMENT_CANCEL,
                                            payment.getCurrency(), payment.getReference(), sourceAccount.getId(),
                                            LedgerBalanceBucket.RESERVED,
                                            sourceAccount.getId(), LedgerBalanceBucket.AVAILABLE,
                                            payment.getAmountMinor());
    ...
    public Payment requestRefund(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.REFUND_REQUESTED);

        outboxService.recordPaymentEvent(payment, OutboxEventType.PAYMENT_REFUND_REQUESTED);
    ...
    public Payment confirmRefund(UUID paymentId) {
        ...
        recordOperation(payment, PaymentOperationType.REFUND_CONFIRMED);

        ledgerService.recordPaymentMovement(payment.getId(), LedgerTransactionType.PAYMENT_REFUND,
                                            payment.getCurrency(), payment.getReference(), targetAccount.getId(),
                                            LedgerBalanceBucket.RESERVED,
                                            sourceAccount.getId(), LedgerBalanceBucket.AVAILABLE,
                                            payment.getAmountMinor());
    ...
    public List<PaymentOperation> getPaymentOperations(UUID paymentId) {
        ...
        return paymentOperationRepository.findAllByPaymentIdOrderByCreatedAtAsc(paymentId);
    }
