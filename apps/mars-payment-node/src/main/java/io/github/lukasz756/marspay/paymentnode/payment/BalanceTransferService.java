...
public class BalanceTransferService {
    ...
    @Transactional
    public BalanceTransfer transfer(
        ...
        BalanceAccount sourceAccount =
                requireBalanceAccount(sourceBalanceAccountId);

        BalanceAccount targetAccount =
                requireBalanceAccount(targetBalanceAccountId);

        requireSameCurrency(sourceAccount, targetAccount);

        BalanceTransfer transfer = BalanceTransfer.create(
                sourceAccount.getId(),
                targetAccount.getId(),
                amountMinor,
        ...
        if (balanceTransferRepository
                .existsBySourceBalanceAccountIdAndReference(
                        sourceAccount.getId(),
                        transfer.getReference()
                )) {
            throw new BalanceTransferAlreadyExistsException(
                    sourceAccount.getId(),
                    transfer.getReference()
            );
        }

        BalanceTransfer savedTransfer =
                balanceTransferRepository.save(transfer);

        sourceAccount.debit(amountMinor);
        targetAccount.credit(amountMinor);

        BalanceOperation debitOperation =
                BalanceOperation.transferDebit(
                        savedTransfer.getId(),
                        sourceAccount.getId(),
                        amountMinor,
        ...
        BalanceOperation creditOperation =
                BalanceOperation.transferCredit(
                        savedTransfer.getId(),
                        targetAccount.getId(),
                        amountMinor,
                        savedTransfer.getReference(),
        ...
        balanceOperationRepository.saveAll(
                List.of(debitOperation, creditOperation)
        );
    ...
    private void requireSameCurrency(
        ...
        if (!sourceAccount.getCurrency().equals(targetAccount.getCurrency())) {
            throw new BalanceTransferCurrencyMismatchException(
                    sourceAccount.getId(),
                    sourceAccount.getCurrency(),
                    targetAccount.getId(),
