...
class PaymentInboxIT extends AbstractPaymentIT {
    ...
    @Test
    void storesAndProcessesDuplicatePaymentEventOnlyOnce() throws Exception {
        ...
        accountService.creditBalanceAccount(sourceAccountId, 1_000, "inbox-funding-" + UUID.randomUUID());
        ...
        PaymentResultPayload payload = new PaymentResultPayload(1, payment.getId(), "AUTHORIZED", null, occurredAt);

        IncomingEventRequest request = new IncomingEventRequest(eventId, "EARTH_PAYMENT_SERVICE", "PAYMENT",
                                                                payment.getId(), "PAYMENT_AUTHORIZED",
        ...
        BalanceAccount sourceAccount = accountService.getBalanceAccount(sourceAccountId);

        assertThat(sourceAccount.getAvailableBalanceMinor()).isEqualTo(700);
