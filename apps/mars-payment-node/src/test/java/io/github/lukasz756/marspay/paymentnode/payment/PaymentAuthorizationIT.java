...
class PaymentAuthorizationIT extends AbstractPaymentIT {
    ...
    @Test
    void rejectsAuthorizationWhenBalanceIsInsufficientAndRollsBackAllChanges() throws Exception {
        ...
        accountService.creditBalanceAccount(sourceAccountId, 100, "authorization-funding-" + UUID.randomUUID());

        Payment payment = paymentService.createPayment(sourceAccountId, targetAccountId, 300,
                                                       "insufficient-balance-" + UUID.randomUUID());

        long outboxCountBefore = countOutboxEvents(payment.getId());

        assertThat(ledgerService.getPaymentLedger(payment.getId())).isEmpty();
        ...
        mockMvc.perform(post("/api/payments/{paymentId}/authorize", payment.getId()))
                ...
                .andExpect(jsonPath("$.detail").value("Insufficient balance on account %s: ".formatted(
                        sourceAccountId) + "available=100, requested=300"));

        mockMvc.perform(get("/api/payments/{paymentId}", payment.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("CREATED"));

        BalanceAccount sourceAccountAfterFailure = accountService.getBalanceAccount(sourceAccountId);

        BalanceAccount targetAccountAfterFailure = accountService.getBalanceAccount(targetAccountId);

        assertThat(sourceAccountAfterFailure.getAvailableBalanceMinor()).isEqualTo(100);

        assertThat(sourceAccountAfterFailure.getReservedBalanceMinor()).isZero();

        assertThat(targetAccountAfterFailure.getAvailableBalanceMinor()).isZero();

        assertThat(targetAccountAfterFailure.getReservedBalanceMinor()).isZero();
        ...
        assertThat(countOutboxEvents(payment.getId())).isEqualTo(outboxCountBefore);
    }
