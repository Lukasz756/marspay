...
public class RelayMessageService {
    ...
    RelayMessageService(
            RelayMessageRepository relayMessageRepository,
    ...
    public RelayReceiveResult receive(RelayMessageRequest request) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Cannot serialize relay message payload",
                    exception
            );
        }
    }
    ...
    public List<RelayDeliveryMessage> findReadyMessages(
            Instant now
    ) {
        if (now == null) {
            throw new IllegalArgumentException(
    ...
    private RelayMessage findMessage(UUID eventId) {
        return relayMessageRepository.findById(eventId)
                .orElseThrow(() -> new IllegalStateException(
                        "Relay message not found: " + eventId
                ));
    }
}