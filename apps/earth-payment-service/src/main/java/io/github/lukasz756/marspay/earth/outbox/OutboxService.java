...
public class OutboxService {
    ...
    public void recordPaymentResult(UUID paymentId, OutboxEventType eventType, String reason) {
        if (paymentId == null) {
        ...
        PaymentResultPayload payload = PaymentResultPayload.create(paymentId, eventType, reason);

        try {
            String payloadJson = objectMapper.writeValueAsString(payload);

            OutboxEvent event = OutboxEvent.pendingPayment(paymentId, eventType, payloadJson);

            outboxEventRepository.save(event);
        } catch (JacksonException exception) {
            throw new IllegalStateException("Cannot serialize payment result event", exception);
        }
    }
    ...
    public List<OutboxMessage> findReadyEvents(Instant now) {
        ...
        return outboxEventRepository.findTop50ByStatusAndAvailableAtLessThanEqualOrderByAvailableAtAscCreatedAtAsc(
    ...
    private OutboxEvent findEvent(UUID eventId) {
        return outboxEventRepository.findById(eventId)
                .orElseThrow(() -> new IllegalStateException("Outbox event not " +
                                                                     "found: " + eventId));
    }
}